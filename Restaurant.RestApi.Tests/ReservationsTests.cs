...
namespace Ploeh.Samples.Restaurants.RestApi.Tests
{
    public sealed class ReservationsTests
    {
        ...
        {
            // URL이 모두 소문자로만 되어 있으면 관용적으로 형식이 지정된 것으로
            // 간주합니다. 이 규칙에서 제외되는 것은 쿼리 문자열 값이며, 이 값은
            // 런타임 데이터를 포함할 수도 있으며, 이 API에서는 Base64 형식으로
            // 인코딩된 HMAC 시그니처를 포함할 수도 있습니다. 
        ...
        public async Task NoHackingOfUrlsAllowed(string id)
        {
            ...
            /* 이 부분은 클라이언트단에서 진행하는 것이 권장되지 않는 일종의 
             * URL 해킹이라 할 수 있습니다. 이 테스트 스위트 역시 다른 
             * 테스트 스위트와 마찬가지로, 링크를 따라가야 합니다.  */
            var urlHack = new Uri($"/reservations/{id}", UriKind.Relative);
            var getResp = await api.CreateClient().GetAsync(urlHack);

            /* 해킹된 URL에 의해서 기대되는 결과는 "403 Forbidden"가 아닌 
             * "404 Not Found". 클라이언트는 단순히 존재하지 않는 URL을 
             * 요청한 것이기 때문입니다. */
